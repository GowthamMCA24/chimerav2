    ...
    <div class="container">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-12 lg:gap-[100px]">
            ...
            <div class="main-content-column">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10 pb-5 border-b border-lightGray">
                    <!-- Author Info -->
                    ...
                    <div class="flex flex-col sm:items-end">
                        <span class="text-gray font-sans text-sm font-semibold capitalize tracking-wider mb-0.5">Published</span>
                        <span class="text-gray font-sans text-xs font-medium"><?php echo esc_html( get_the_date( 'F j, Y' ) ); ?></span>
                    </div>
                </div>
                <?php endif; ?>
            ...
            <aside class="sidebar-column">
                ...
                <div>
                 ...
                 <!-- Sidebar CTA Widget -->
                <div class="sidebar-cta bg-orange rounded-[10px] py-5 px-[30px] text-center shadow-lg relative overflow-hidden text-white">
                    <?php 
                    ...
                    ?>
                    <h3 class="font-jost text-lg font-semibold text-white mb-4 leading-tight"><?php echo $toc_title ?? 'Some good heading here...'; ?></h3>
                    <a href="<?php echo esc_attr($toc_btn_link); ?>" <?php echo $btn_onclick; ?> class="inline-flex items-center gap-2 bg-white text-orange font-semibold px-6 py-3 rounded-[20px] text-sm hover:bg-gray-50 transition-all duration-200 group shadow-md">
...
<div id="whitepaper-popup" class="fixed inset-0 z-[100] hidden">
    <!-- Backdrop -->
    <div class="absolute inset-0 bg-dark/40 backdrop-blur-[15px]" onclick="document.getElementById('whitepaper-popup').classList.add('hidden');"></div>
    
    <!-- Modal -->
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[500px] bg-white rounded-[16px] shadow-2xl p-8 sm:p-10 z-10 flex flex-col">
        
        <!-- Close Button -->
        <button onclick="document.getElementById('whitepaper-popup').classList.add('hidden');" class="absolute top-4 right-4 text-gray hover:text-dark transition-colors" aria-label="Close">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke-width="1.5"></circle><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 9l-6 6M9 9l6 6"></path></svg>
        </button>
        ...
        <div id="whitepaper-form-wrapper">
            <form id="whitepaper-form" action="#" method="POST" class="space-y-4">
                ...
                <input type="hidden" name="whitepaper_file" value="<?php echo esc_url(get_field('whitepaper_file')); ?>">
                <input type="hidden" name="whitepaper_title" value="<?php echo esc_attr(get_the_title()); ?>">

                <div>
                    <input type="text" name="name" required placeholder="Jhon Doe *" class="w-full px-4 py-3 border border-bordergray rounded-[8px] focus:outline-none focus:border-orange focus:ring-1 focus:ring-orange transition-all text-sm text-dark placeholder:text-gray/70 font-medium">
                </div>
                <div>
                    <input type="email" name="work_email" required placeholder="Work Email *" class="w-full px-4 py-3 border border-bordergray rounded-[8px] focus:outline-none focus:border-orange focus:ring-1 focus:ring-orange transition-all text-sm text-dark placeholder:text-gray/70">
                </div>
                <div>
                    <input type="text" name="company" placeholder="Company" class="w-full px-4 py-3 border border-bordergray rounded-[8px] focus:outline-none focus:border-orange focus:ring-1 focus:ring-orange transition-all text-sm text-dark placeholder:text-gray/70">
                </div>
                
                <div class="flex items-start gap-3 mt-4 pt-2">
                    <input type="checkbox" id="consent" checked required class="mt-0.5 w-4 h-4 accent-orange text-orange rounded cursor-pointer">
                ...
                <button type="submit" class="w-full bg-orange text-white font-semibold py-3.5 rounded-[30px] mt-6 flex items-center justify-center gap-2 hover:bg-orangeLight transition-colors">
        ...
        <div id="whitepaper-success" class="hidden text-center py-8">
            ...
            <p class="text-gray text-sm font-normal">Please check your email. We've sent the download link to your inbox.</p>
        </div>
